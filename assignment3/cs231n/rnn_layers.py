...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    H = b.shape[0]
    h = np.zeros((N,T,H))
    prev_h = np.zeros((h.shape))
    z = np.zeros((h.shape))
    previous = h0
    h[:,-1,:] = h0
    for t in range(T):
        h[:,t,:], (x[:,t,:],Wx,prev_h[:,t,:],Wh,b,z[:,t,:]) = rnn_step_forward(x[:,t,:],previous, Wx, Wh, b)
        previous = h[:,t,:]
    pass
    cache = (x,Wx,prev_h,Wh,b,z)
    ##############################################################################
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(np.arange(T)):
        dh_t = dh[:,t,:]+ dprev_t
        dx[:,t,:],dprev_t,dWx_t,dWh_t,db_t = rnn_step_backward(dh_t,(x[:,t,:],Wx,prev_h[:,t,:],Wh,b,z[:,t,:]))
        dh0 = dprev_t
        dWx+=dWx_t
        dWh+=dWh_t
        db+=db_t
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        next_h[:,t,:],next_c[:,t,:],c =lstm_step_forward(
            x[:,t,:],previous_h,previous_c, Wx, Wh, b)
        previous_h = next_h[:,t,:]
        previous_c = next_c[:,t,:]
        cache.append(c)
    pass
