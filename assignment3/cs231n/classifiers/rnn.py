...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            dfeatures,grads['W_proj'],grads['b_proj'] = affine_backward(dout1,cache1)
        else:
            out1,cache1 = affine_forward(features, W_proj, b_proj)
            out2,cache2 = word_embedding_forward(captions_in, W_embed)
    ...
    def sample(self, features, max_length=30):
        ...
        next_c = np.zeros((out1.shape))
        capt=self._start* np.ones((N, 1), dtype=np.int32)
        for t in range(max_length):
            if t == max_length-1:
                captions[:,29]=self._end
                break
            out2,cache2 = word_embedding_forward(capt, W_embed)
            if self.cell_type == 'rnn':
                out3,cache3 = rnn_step_forward(out2[:,0,:], out1, Wx, Wh, b)
            else:
                out3,next_c,cache3 = lstm_step_forward(out2[:,0,:], out1,next_c, Wx, Wh, b)
            out4,cache4 = temporal_affine_forward(out3.reshape(N,1,512), W_vocab, b_vocab)
            captions[:,t] = np.squeeze(np.argmax(out4,axis=2))
            capt = captions[:,t]* np.ones((N, 1), dtype=np.int32)
            out1 = out3
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
